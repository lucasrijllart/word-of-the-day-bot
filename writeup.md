...
time of codind there was no official API to use. I found multiple articles pointing to
unofficial libraries and tools that can be used, and every time there were lots of warnings
such as: "This is an unofficial library, use at your own risk". Many comments were made online
about how Instagram was actively banning bot accounts.

I found this library and decided to give it a go: https://github.com/ohld/igbot.
I created a quick Instagram template for the picture I wanted to post, then wrote the
integration code, which was very simple. Here is what it looked like:
...
However, in 2021 it seems Facebook has now introduced new APIs that allows for Instagram
programming. These are the same as the Facebook Graph API but for the picture-only social
...
can be accessed by a large number of approved unofficial libraries for your language of
...
worked for a few days, until one day, Twitter restricted my application. I contacted their
...
so that it wouldn't be caught in this process. I found this incredibly stupid, given that the
API key you are given is generated by Twitter in the first place, so it doesn't make sense to
then scour the keys they've generated and disable "abusive" ones. After changing my key,
...
## Conclusion
This project brought in the right number of challenges I wanted, and also provided 
